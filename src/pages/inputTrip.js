...
class InputTrip extends React.Component {
  ...
  handleSubmit = async (e) => {
    ...
    try {
      ...
      await addDoc(lokasiAwalRef, {
        ...
        alamat: namaLokasi.village + ", " + namaLokasi.city,
        lokasi: lokasi.value,
      });
      Swal.fire({
        title: "Berhasil",
  ...
  handleChangeLokasiTerakhir = async (nilai) => {
    await new Promise((resolve) => {
      this.setState({ lokasiAwalSelect: nilai }, resolve);
    });
  ...
  handlePerjalananLanjut = async (e) => {
    e.preventDefault();
    try {
      const { alasan, jamBerangkat, status, user, hariIni, lokasiAwalSelect } =
        this.state;
                ...
                <Select
                  ...
                  classNames={{
                    menuButton: ({ isDisabled }) =>
                      `ps-3 text-[15px] flex text-sm text-blue-500 w-[100%] bg-blue-100 rounded shadow-sm transition-all duration-300 focus:outline-none ${
                        isDisabled
                          ? "bg-blue-100"
